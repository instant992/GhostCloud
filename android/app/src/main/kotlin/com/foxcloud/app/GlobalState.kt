...
import com.foxcloud.app.plugins.AppPlugin
import com.foxcloud.app.plugins.TilePlugin
import com.foxcloud.app.plugins.VpnPlugin
...
object GlobalState {
    ...
    const val NOTIFICATION_CHANNEL = "GhostCloud"
    const val SUBSCRIPTION_NOTIFICATION_CHANNEL = "GhostCloud_Subscription"
    ...
    fun hasActiveProfile(): Boolean {
        val prefs = FoxCloudApplication.getAppContext()
    ...
    fun getCurrentTilePlugin(): TilePlugin? {
        // Tile listener is registered ONLY in _service (runs on serviceEngine).
        // The main Dart isolate (flutterEngine) has NO tile listener,
        // so we must NEVER return TilePlugin from flutterEngine.
        // Returning null triggers initServiceEngine() + pending action flow.
        return serviceEngine?.plugins?.get(TilePlugin::class.java) as TilePlugin?
    }
