...
var
  ...
  ResultCode: Integer;
begin
  Processes := ['FoxCloud.exe', 'FoxCloudCore.exe', 'FoxCloudHelperService.exe'];
...
begin
  ...
  Exec('sc.exe', 'stop "FoxCloudHelperService"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Sleep(1000);
...
begin
  ...
  begin
    ...
    try
      Exec('sc.exe', 'start "FoxCloudHelperService"', '', SW_HIDE, ewNoWait, ResultCode);
    except
    end;
  end;
end;
...
begin
  case CurUninstallStep of
    ...
    begin
      ...
      // Delete service
      Exec('sc.exe', 'delete "FoxCloudHelperService"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Sleep(500);
    end;
    
    usPostUninstall:
    begin
      if DirExists(ExpandConstant('{userappdata}\com.foxcloud\\app')) then
      begin
        if MsgBox('Удалить пользовательские данные программы?', mbConfirmation, MB_YESNO) = IDYES then
        begin
          DelTree(ExpandConstant('{userappdata}\com.foxcloud\\app'), True, True, True);
        end;
      end;
    end;
  end;
