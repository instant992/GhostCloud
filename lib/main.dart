...
import 'package:foxcloud/plugins/tile.dart';
...
Future<void> main() async {
  ...
  HttpOverrides.global = FoxCloudHttpOverrides();
  runApp(const ProviderScope(
    child: Application(),
  ));

  // Initialize Firebase & FCM for push notifications AFTER runApp
  // so the app UI starts immediately and doesn't hang on Firebase init.
  if (Platform.isAndroid) {
    Future(() async {
      try {
...
Future<void> _service(List<String> flags) async {
  ...
  // Initialize VPN plugin handler in the service engine's Dart isolate.
  // VpnPlugin (Kotlin) sends "getStartForegroundParams" and "status" via
  // the service engine's BinaryMessenger. Without this, those calls have
  // no Dart handler and hang forever → no foreground notification → Android
  // kills the service → VPN stops working.
  if (Platform.isAndroid) {
    vpn;
    commonPrint.log("[DART] Vpn handler initialized in service engine");
  }
  ...
  tile?.addListener(
    _TileListenerWithService(
      onStart: () async {
        ...
        try {
          commonPrint.log("TileService: Starting VPN");
          unawaited(app?.tip("Запуск VPN..."));
  ...
  vpn?.handleGetStartForegroundParams = () {
    try {
      ...
      final profileName = profile?.label ?? profile?.id ?? "GhostCloud";
      ...
      // Service name (subtext) from header foxcloud-servicename (constant per profile)
      String serviceName = "";
      try {
        String? svc = profile?.providerHeaders['foxcloud-servicename'];
