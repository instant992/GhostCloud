...
import 'widgets/tg_proxy_banner.dart';
...
class _DashboardViewState extends ConsumerState<DashboardView> with PageMixin {
        ...
        Consumer(
          builder: (context, ref, child) {
            final denyEditing = ref.watch(currentProfileProvider
                .select((profile) => profile?.providerHeaders['foxcloud-denywidgets']));
  ...
  bool _isAllowedWidget(
    ...
    // Hide blocks moved to Subscription tab
    if (item == DashboardWidget.metainfo ||
        item == DashboardWidget.outboundMode ||
        item == DashboardWidget.outboundModeV2 ||
        item == DashboardWidget.serviceInfo) {
      return false;
    }
        ...
        Expanded(
          child: Align(
            ...
            child: SingleChildScrollView(
              ...
              child: _buildIsEdit((isEdit) => Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (!isEdit) const TgProxyBanner(),
                      isEdit
                        ? SystemBackBlock(
