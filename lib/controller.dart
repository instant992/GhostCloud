...
import 'package:foxcloud/common/archive.dart';
import 'package:foxcloud/services/subscription_notification_service.dart';
import 'package:foxcloud/services/push_notification_service.dart';
...
class AppController {
  ...
  void initForegroundCache() {
    ...
    String serviceName = "";
    final svc = profile.providerHeaders['foxcloud-servicename'];
    ...
    String? groupName = profile.providerHeaders['foxcloud-serverinfo'];
  ...
  Future<void> deleteProfile(String id) async {
    ...
    savePreferencesDebounce();
    // If no profiles left, reset auth to show login screen
    if (globalState.config.profiles.isEmpty) {
      _ref.read(foxAuthProvider.notifier).logout();
    }
  }
  ...
  void _applyAllHeaderSettings(Profile profile, {required bool isNewProfile}) {
    ...
    final customBehavior = headers['foxcloud-custom'];
  ...
  void _applyProviderSettings(Map<String, String> headers) {
    try {
      ...
      final settingsHeader = headers['foxcloud-settings'];
  ...
  void _applyThemeColor(Map<String, String> headers) {
    try {
      final hexHeader = headers['foxcloud-hex'];
  ...
  void _applyThemeColorFromHex(String hexHeader) {
    try {
      ...
      commonPrint
          .log('Applying theme from foxcloud-hex: #${hexString.toUpperCase()}'
  ...
  Future<void> updateProfile(Profile profile) async {
    ...
    // Check for new push notifications from admin panel
    unawaited(PushNotificationService.checkAndShow().catchError((e) {
      commonPrint.log("Error checking push notifications: $e");
    }));
  }
  ...
  Future<void> init() async {
    ...
    // Check for push notifications shortly after launch
    Future.delayed(const Duration(seconds: 3), () {
      unawaited(PushNotificationService.checkAndShow().catchError((e) {
        commonPrint.log("Error checking push notifications on init: $e");
      }));
    });
    autoUpdateProfiles();
    autoCheckUpdate();
  ...
  Future<void> addProfileFormURL(String url) async {
    ...
    try {
      ...
      if (profile != null) {
        ...
        // Immediately save to disk so profile survives app kill from recents
        await savePreferences();
      }
    } catch (err) {
  ...
  void _applyCustomViewSettings(Profile profile) {
    ...
    final dashboardLayout = headers['foxcloud-widgets'];
    ...
    final proxiesView = headers['foxcloud-view'];
